...
class Pokemon:
    ...
    def golpear (self,P) :
        ...
        if random.random () <= (self.Velocidad - P.Velocidad ) / 100 : 
            ...
            Daño = max ([( self.Ataque - P.Defensa ) + random.randrange(-10 ,11) ,1])
            print ("Golpe certero de", self.Nombre)
            print("El daño realizado es: " + str(Daño))
        else:
            print (P.Nombre , "esquiva el golpe !")

def Simular_batalla_pokemon(j1,j2):

    #Comienda el pokemon con mayor velocidad
    
    if (j1.Velocidad < j2.Velocidad):
        Atacante,Receptor = j2,j1
    else:
        Atacante,Receptor = j1,j2
    #Se realiza el combate hasta que alguno se quede sin vida
    while(j1.Puntos_de_salud > 0 and j2.Puntos_de_salud > 0):
        print("\n" + j1.Nombre,j1.Puntos_de_salud,"vs", 
                j2.Puntos_de_salud, j2.Nombre)
...
Simular_batalla_pokemon(Bulbasaur, Squirtle)
